...
public void setup() {
  ...
  polygonPoints.add(2, 0.15,"h");
  polygonPoints.add(6, 0.12, "q");
  polygonPoints.add(15, 0.3, "w");
  polygonPoints.add(8, 0.6, "g");
  polygonPoints.add(1.5, 0.5, "s");
...
public void draw() {
  ...
  if (index >= 0 && index < gaussianStack.length) {
    ...
    for (int i = 0; i < gaussianStack.length; i++) {
      //points3.add(i + 0.5 - gaussianStack.length/2.0, gaussianStack[i]/gaussianCounter, "H" + i);
      points3.add(i, gaussianStack[i]/gaussianCounter, "H" + i);
    }
